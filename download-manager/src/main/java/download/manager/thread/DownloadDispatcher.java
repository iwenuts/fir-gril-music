...
import download.manager.thread.util.Log;
...
class DownloadDispatcher extends Thread {
    ...
    private void executeDownload(DownloadRequest request, String downloadUrl) {
        if (request.fakeUrl()) {
            boolean res = request.getRealUrl();
            if (!res) {
                updateDownloadFailed(request, HTTP_UNAVAILABLE, "Get Download Url Failed");
                return;
            }
            downloadUrl = request.getUri().toString();
        }
        if (request.iCanDownload()) {
            mContentLength = 1;
    ...
    private int readFromResponse(DownloadRequest request, byte[] data, InputStream entityStream) {
        try {
            ...
            // 该异常表明：http header中的 content-length 字段值 与 实际流大小 不符
    ...
    void updateDownloadState(DownloadRequest request, int state) {
        request.setDownloadState(state);
    }

    void updateDownloadComplete(DownloadRequest request) {
    ...
    void updateDownloadFailed(DownloadRequest request, int errorCode, String errorMsg) {
        mDownloadedCacheSize = 0; // reset into Zero.
        shouldAllowRedirects = false;
    ...
    void updateDownloadProgress(DownloadRequest request, int progress, long downloadedBytes) {
