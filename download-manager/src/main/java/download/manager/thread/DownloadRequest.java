package download.manager.thread;
...
public class DownloadRequest implements Comparable<DownloadRequest> {
    ...
    private boolean fakeUrl = false;

    public void setFakeUrl(boolean fakeUrl) {
        this.fakeUrl = fakeUrl;
    }

    public final boolean fakeUrl() {
        return fakeUrl;
    }

    final boolean getRealUrl() {
        boolean res = onGetRealUrl();
        if (res) {
            setFakeUrl(false);
        }
        return res;
    ...
    protected boolean onGetRealUrl() {
        return false;
    }


    protected void updateDownloadState(int state) {
        if (mDispatcher != null) {
            mDispatcher.updateDownloadState(this, state);
        }
    }

    protected void updateDownloadComplete() {
        if (mDispatcher != null) {
            mDispatcher.updateDownloadComplete(this);
        }
    }

    protected void updateDownloadFailed(int errorCode, String errorMsg) {
        if (mDispatcher != null) {
            mDispatcher.updateDownloadFailed(this, errorCode, errorMsg);
        }
    }

    protected void updateDownloadProgress(int progress, long downloadedBytes) {
        if (mDispatcher != null) {
            mDispatcher.updateDownloadProgress(this, progress, downloadedBytes);
        }
    }
